...
/* ---------------- sync from n8n (Google Sheets via bookings.json on GitHub) ---------------- */
// بيجيب أحدث الحجوزات من ملف bookings.json (اللي n8n بيحدّثه كل شوية من جوجل شيت)
// الملف لازم يكون موجود في نفس مجلد index.html على GitHub Pages
const BOOKINGS_JSON_PATH = 'bookings.json';

async function fetchBookingsFromSheet(silent){
  try{
    // ?t=Date.now() عشان نمنع كاش المتصفح ونجيب أحدث نسخة من الملف كل مرة
    const res = await fetch(BOOKINGS_JSON_PATH + '?t=' + Date.now());
    if(!res.ok) throw new Error('HTTP '+res.status);
    const data = await res.json();
    // الملف ممكن يكون array مباشرة أو object فيه مفتاح bookings/data
    ...
    if(!silent) showToast('فشل تحديث الحجوزات — تأكد من وجود bookings.json في الريبو');
...
function mergeBookingsFromSheet(rows){
  rows.forEach(row=>{
    if(!row.patientName && !row.date && !row.time) return; // تجاهل صفوف فاضية
    // استخدم id لو موجود من الشيت، وإلا اعمل مطابقة بالاسم+التاريخ+الوقت
...
function startAutoSync(){
  stopAutoSync();
  fetchBookingsFromSheet(true); // أول تحديث فورًا لما الموقع يفتح
...
(async function init(){
  ...
  render();
  startAutoSync(); // يبدأ تحديث الحجوزات من bookings.json تلقائيًا كل 30 ثانية
})();
